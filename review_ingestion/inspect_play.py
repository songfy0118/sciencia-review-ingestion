...
def export_database(db: Path, output: Path) -> dict:
    ...
    rows = "".join(
        f'<tr><td>{escape(r["app_id"])}</td><td>{r["score"]}/5</td>'
        f'<td>{escape(r["review_at"])}</td><td class="content">{escape(r["content"])}</td>'
        f'<td>{escape(r["review_id"])}</td></tr>' for r in records[:100]
    )
...
<section><h2>Review preview</h2><p>Showing up to 100 records, grouped by app. Download JSON for all stored reviews.
...
<tbody>REVIEW_ROWS</tbody></table></div></section></main></html>'''
    document = document.replace("RECORD_COUNT", str(len(records))).replace("APP_SUMMARY", summary).replace("REVIEW_ROWS", rows)
